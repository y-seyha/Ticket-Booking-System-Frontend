...
import { listAllBookings, getBookingsExportUrl } from '@/features/bookings/bookings.api';
import type { AdminBooking, AdminBookingListResponse } from '@/features/bookings/bookings.types';
...
export default function AdminBookingsPage() {
  ...
  const [meta, setMeta] = useState<AdminBookingListResponse['meta'] | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError('');
      try {
        const res = await listAllBookings({ status: status || undefined, search: debouncedSearch || undefined, from, to });
        ...
        setData([]);
        setMeta(null);
        setError((err as { response?: { data?: { message?: string } }; message?: string })?.response?.data?.message || (err as Error)?.message || 'Failed to load bookings');
      } finally {
    ...
    fetchData();
  }, [status, debouncedSearch, from, to, refreshKey]);
  ...
  const summary = {
    ...
    expired: data.filter((b: AdminBooking) => b.status === 'EXPIRED').length,
  };
  ...
  return (
    <div className="space-y-6 px-6 py-8 sm:px-8 sm:py-10 max-w-7xl mx-auto min-h-screen">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-zinc-100 dark:border-zinc-800 pb-5">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">Bookings</h1>
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">Manage all ticket bookings</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <a
            href={getBookingsExportUrl({ status: status || undefined, from, to })}
            download
      ...
      <BookingSummaryCards {...summary} loading={showLoading} />
      <BookingFilters status={status} search={search} from={from} to={to} onStatusChange={setStatus} onSearchChange={setSearch} onFromChange={setFrom} onToChange={setTo} />
      ...
      <AdminBookingsTable data={data} loading={showLoading} onSelect={setSelectedId} />
      ...
      <BookingDetailModal bookingId={selectedId} onClose={() => setSelectedId(null)} onCancelled={() => {
        setRefreshKey(k => k + 1);
      }} />
    </div>
  );
